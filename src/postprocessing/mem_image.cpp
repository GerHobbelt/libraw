...
ushort scale2GL_16F(ushort input);
  ...
  for (row = 0; row < S.height; row++, soff += rstep)
  {
    ...
    if (bgr)
    {
      ...
      else
      {
        for (col = 0; col < S.width; col++, soff += cstep)
          FORBGR *ppm2++ =
            scale2GL_16F(imgdata.color.curve[imgdata.image[soff][c]]);
      }
    }
    else
    {
...
ushort float2half(float inputNum)
{
  unsigned int input;
  memcpy(&input, &inputNum, sizeof(inputNum));

  static const ushort base_table[512] = {
  ...
  ushort hbits =
      base_table[input >> 23] +
      static_cast<ushort>((input & 0x7FFFFF) >> shift_table[input >> 23]);

  hbits += (((input & 0x7FFFFF) >> (shift_table[input >> 23] - 1)) |
            (((input >> 23) & 0xFF) == 102)) &
           ((hbits & 0x7C00) != 0x7C00);
...
ushort scale2GL_16F(ushort input) { return float2half(input / 65535.0); }